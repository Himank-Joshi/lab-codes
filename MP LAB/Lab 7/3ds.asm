...
DATA SEGMENT
  arr DW 1, -4, 2, 5, -3 ; example array of signed words
...
; function to perform selection sort on an array of signed words
sort_signed_words:
...
outer_loop:
  ...
  MOV AX, [arr+BX*2] ; initialize smallest value to current value
  ...
  inner_loop:
    ...
    MOV DX, [arr+CX*2] ; load value of next element
  ...
  swap:
    MOV DX, [arr+BX*2] ; move current value to temporary location
    MOV [arr+BX*2], AX ; move smallest value to current location
    MOV [arr+CX*2], DX ; move current value to smallest location
